import { BrushTimeline, BrushTimelineVars, EaseInput } from './animation/engine';
export type AnnotationType = "highlight" | "underline" | "box" | "circle" | "contour" | "strike-through" | "crossed-off" | "bracket";
export type BracketSide = "left" | "right" | "top" | "bottom";
export type Padding = number | [vertical: number, horizontal: number] | [top: number, right: number, bottom: number, left: number];
/** A point in annotation-local CSS pixels: [x, y, pressure?] */
export type BrushPoint = [number, number, number?];
export type PressureConfig = number | [min: number, max: number] | ((t: number) => number);
