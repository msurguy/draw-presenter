...
function applyLoadedTarget(target, width, height, density) {
  ...
  // Reuse the renderer when reloading the same canvas: compositor resources
  // (blend shader, mask framebuffers) are cached on the renderer object, so a
  // fresh object per load() would recompile/reallocate them on every call and
  // leak the previous framebuffers.
  if (activeRenderer?.canvas !== target) {
    activeRenderer = createRenderer(target, width, height, density);
  } else {
    activeRenderer.width = width;
    activeRenderer.height = height;
    activeRenderer.density = density;
  }
  setTargetState({
