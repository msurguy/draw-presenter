...
uniform mat4 u_proj;
// Fraction of the tip texture's half-width actually covered by ink (0..1].
// Sampling only that central region lets small tip drawings fill the stamp.
...
void main() {
    ...
    // Map corner [-1,1] → uv [0,1] (scaled to the inked region); without
    // UNPACK_FLIP_Y this gives the correct orientation because our projection
    // already has Y increasing downward.
