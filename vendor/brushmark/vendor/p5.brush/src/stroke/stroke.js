...
export function add(name, params) {
  ...
  if (params.type === "custom") {
    ...
    g.noStroke();
    // Tips are defined in dark tones on white paper (dark → opaque). Default
    // the fill to black so documented tips that never call fill() are visible.
    g.fill(0);
    params.tip(g);
    g.pop();
...
const T = {
  ...
  imageToWhite(image) {
    ...
    if (maxX >= 0) {
      const cx = w / 2;
      const cy = h / 2;
      const extent = Math.max(cx - minX, maxX + 1 - cx, cy - minY, maxY + 1 - cy);
      image.uvFit = Math.min(1, Math.max(0.02, (extent + 2) / cx));
    } else {
      image.uvFit = 1;
    }
  },
