...
export function glDrawImages(p5img, src) {
  ...
  if (!tex) {
    ...
    // Stamps are usually much smaller than the tip buffer; without mipmaps
    // minification point-samples the tip and thin ink shapes disappear.
  ...
  // Sample only the tip's inked region (measured in imageToWhite) so small
  // tip drawings fill the stamp instead of shrinking to a sub-pixel dot.
  gl.uniform1f(imgFrag.u_uvScale, p5img.uvFit ?? 1);
